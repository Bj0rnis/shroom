# home-server Shroom — build pause / handoff
...
- **Branch:** `shroom/build`
- **PR:** [home-server#25](https://github.com/saniakhushbakhtjamil/home-server/pull/25) (draft)
...
to the server (`shroom.home-server` is dark). All work has been local against
`http://localhost:3000`.
...
2. **Scaffold.** Stack at `stacks/shroom/` from the dashboard template.
   Express + kit, traefik labels for `shroom.home-server`, JSON volume mount.
...
- Server data target: `/opt/home-server/data/shroom/` (will be created on first
...
MOCK=true TICK_INTERVAL_MS=1000 NIGEHBAN_INTERVAL_TICKS=20 \
  ...
  node /Users/the maintainer/Documents/Projects/home-server/stacks/shroom/app/server.js
```
...
[ ] Manual toofan works
[ ] Reachable at https://shroom.home-server over Tailscale  ← step 11
[ ] Smell test                                         ← step 11
```
...
gh pr ready 25 --repo saniakhushbakhtjamil/home-server
gh pr merge  25 --repo saniakhushbakhtjamil/home-server --merge
...
curl -k https://shroom.home-server/api/health
```
