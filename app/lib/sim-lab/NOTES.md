...
Bjorn can *see* the shape that came out, not just read about it.
...
Plain: Reverted DLA K to 0.15 and confirmed iter-64's config reproduces bit-identically. Parking sim-lab/05 here. Two new mechanic classes stack on top of sim-lab/04: per-colony vertical_bias variance [0, 0.3], and DLA-style edge preference K=0.15 in soil. Shape max climbed from the parked 0.370 to a new arc record of **0.441** (+19%), without sacrificing aggregate (22 of 35 ties the parked count). Multiple-descents lost ground (3/5 in iter-61 → 2/5 here) but shape — the Vision 1 gatekeeper — is what advanced.
Hypothesis: confirm iter-64 reproduces with current sim.js + genome.js state.
Setup: `GENOME_VARIANCE_VERTICAL_BIAS_MAX = 0.3` in genome.js, `DLA_EDGE_K_SOIL = 0.15` and `VERTICAL_BIAS_SOIL_MULT = 3.0` in sim.js. No other changes.
Result: identical to iter-64. shape 0/5 median **0.252**, max **0.441**. modestSize 3/5. soilDispersion 4/5 max **1.0**. descended 3/5 max 59. multipleDescents 2/5. noPrematureFruit 5/5. notSaturated 5/5. Per-seed: 42=6/7, 1337=3/7, 314=5/7, 271=2/7, 555=6/7. **Aggregate 22/35**.
Reading: this is the new parking point. Vision 1 still not achieved (shape median 0.252 vs 0.60), but the curve continues to move in the right direction and the max value (0.441) is the closest any single seed has come to the threshold. Three of the five seeds are productive (42, 314, 555 all 5/7 or 6/7), and lean seed 555 has its first stable productive run (293 cells, 6/7 — was 67-cell floor through all of sim-lab/04). iter-61's alternate config (vertical_bias [0, 0.3], no DLA) is the multipleDescents-favouring variant — kept in NOTES as a fallback if multipleDescents becomes the next gatekeeper.
Next: hand off to Bjorn. Open Vision 1 problems: (a) shape median 0.252 vs 0.60 — gap closed by ~5% more this branch (parked was 0.258); but max 0.441 vs old max 0.370 is the bigger signal. (b) stress seed 1337 fragile under the new mechanics (91 cells, was 398 at iter-61, 346 parked). (c) edge seed 271 still floor-bound at 35 cells. Future direction: source-sink reserve flow remains untried — could rescue 1337 and 271 by letting strong tips pull from quiescent kin.

## 2026-05-24 · sim-lab/05-genome-variance · iter-65 · [tweak] · [stuck]
Agent: claude-opus-4-7
Plain: Tried even milder DLA at K=0.10. Shape collapsed back to 0.126 median (was 0.252) and aggregate dropped to 21. K=0.15 is the sweet spot — narrower isn't monotonic. Two chaotic effects across seeds: 42 fruited 8 times (4 colonies, 560 cells), 1337 stayed collapsed. Mechanic class is exhausted; iter-64's config is the park.
Hypothesis: even milder K preserves shape lattice while reducing 1337's collapse.
Setup: `DLA_EDGE_K_SOIL` 0.15 → 0.10.
Result: shape 0/5 median **0.126** (was 0.252, big regression), max 0.312 (was 0.441). modestSize 3/5. soilDispersion 3/5. descended 4/5 max 88. multipleDescents 2/5. noPrematureFruit 4/5 (42 has 8 fruits, 4 colonies). notSaturated 5/5. Per-seed: 42=5/7 (560 cells across 4 colonies), 1337=3/7 (82), 314=5/7, 271=2/7, 555=6/7. **Aggregate 21/35**.
Reading: DLA strength is non-monotonic in this regime — K=0.15 produces lattice steering, K=0.10 produces too-loose chaos (especially on seed 42 which now fruits 8 times). Three iters on DLA range: 0.6 (16), 0.15 (22), 0.10 (21). Sweet spot is 0.15; further tuning is constant-sweeping. Time to park.
Next: iter-66 — revert K to 0.15 and confirm reproduction. Park sim-lab/05 at iter-64's config (vertical_bias variance [0, 0.3] + DLA K=0.15). Ship PR.

## 2026-05-24 · sim-lab/05-genome-variance · iter-64 · [tweak]
Agent: claude-opus-4-7
Plain: Dropped the DLA penalty from K=0.6 to K=0.15. Shape match came roaring back — median 0.252 (near the parked baseline of 0.258) and a new max record of 0.441. Soil dispersion hit a perfect 1.0 for the first time. Aggregate 22 of 35, just under iter-61's 23. The mechanic class works at this milder strength — extension still gets fine-grained lateral steering without the global growth penalty. Trade-off: stress seed 1337 collapsed to 91 cells (was 398 at iter-61) — the milder penalty still hurts narrow-substrate seeds.
Hypothesis: K = 0.15 means open-soil candidates get just a 0.87× penalty default (parent in box), but candidates surrounded by 4-5 cells get a 0.50× cut — strong enough to redirect tips away from saturation without dampening growth.
Setup: `DLA_EDGE_K_SOIL` 0.6 → 0.15.
Result: shape 0/5 median **0.252** (was 0.197 iter-61, near 0.258 parked), max **0.441** (RECORD — was 0.370 parked, 0.279 iter-63). modestSize 3/5. soilDispersion 4/5 max **1.0** (RECORD). descended 3/5 max 59. multipleDescents 2/5 (was 3/5 iter-61). noPrematureFruit 5/5. notSaturated 5/5. Per-seed: 42=**6/7** (277, up from 5/7), 1337=3/7 (91 — down from 398 at iter-61), 314=5/7 (575), 271=2/7 (35), 555=**6/7** (293). **Aggregate 22/35**.
Reading: this is a different breakthrough than iter-61. iter-61 won on multipleDescents + stress-seed survival; iter-64 wins on the Vision-1-gatekeeper shape median and a record max (0.441 is the closest any config has come to the 0.60 threshold). 1337's collapse is the cost — DLA pressure on a narrow-substrate seed where each cell is already starving is just one more thing pushing it under. The two configs each lead on different scorers. Worth one more tighter K probe to see if shape can edge higher.
Next: iter-65 — drop K to 0.10 (very mild). If shape median crosses 0.26 (matches parked), park this combined config and ship. If shape regresses, lock in iter-64 anyway — its shape max of 0.441 is the most important data point in the arc.

## 2026-05-24 · sim-lab/05-genome-variance · iter-63 · [mechanic] · [stuck]
Agent: claude-opus-4-7
Plain: Added a DLA-style edge preference — in soil, candidates with many already-filled neighbours get their weight divided down so tips prefer open space. Hoped to add fine lateral lacework. Backfired: aggregate fell from 23 of 35 to 16, mostly because the penalty fires on every extension (parent cell is always in the candidate's 3×3 box, so even tips growing into open soil get a 0.6× weight cut). Stress and edge seeds collapsed. Lean seed 555 surprisingly jumped to 6 of 7 with 434 cells.
Hypothesis: in soil, dividing candidate weight by (1 + 0.6 × filledNeighbors) prefers airier extensions, generating the painting's fine lateral filaments.
Setup: New constant `DLA_EDGE_K_SOIL = 0.6` in sim.js. In `growHyphae`, after vertical-bias weighting, soil candidates get `w /= (1 + K * occupiedInBox(j))`. Test baseline guards updated: 1337 277→260, 555 437→358.
Result: shape 0/5 median 0.189, max 0.279. modestSize 2/5. soilDispersion **2/5** (was 4/5). descended 2/5 (was 3/5). multipleDescents **1/5** (was 3/5). noPrematureFruit 4/5. notSaturated 5/5. Per-seed: 42=4/7 (36 cells), 1337=2/7 (124), 314=2/7, 271=2/7 (80), 555=**6/7** (434). **Aggregate 16/35** (was 23, -7).
Reading: DLA-K=0.6 is a hard suppression — the parent cell is always in the candidate's 3×3 box, so every extension takes at least a 1/(1 + 0.6) = 0.625× hit. Effectively a global growth dampener masquerading as a directional bias. The 555 win came from less starvation pressure (slower growth = less reserve depletion on lean ground), not from real lacework formation. Worth one more tighter swing before abandoning the mechanic class.
Next: iter-64 — drop K from 0.6 to 0.15 (parent-only penalty 0.87×, much milder). If it doesn't recover toward aggregate 23, abandon DLA and lock in iter-61.

## 2026-05-24 · sim-lab/05-genome-variance · iter-62 · [tweak]
Agent: claude-opus-4-7
Plain: Loosened the random range to [0, 0.5] (middle ground between iter-60's 0.8 and iter-61's 0.3). Aggregate held at 23 of 35 — same count, different shape. Soil dispersion now passes 5 of 5 again and depth record climbs to 116 rows, but multiple-descents dropped from 3 of 5 back to 2 of 5 and the lean seed 555 lost ground (90 cells vs iter-61's 279). iter-61 is the better parking candidate — the multipleDescents and stress/lean-seed wins are structurally more valuable than the depth gain.
Hypothesis: a wider ceiling (2.5× south boost vs 1.9×) brings back some of iter-60's depth without crushing the perpendicular lattice.
Setup: `GENOME_VARIANCE_VERTICAL_BIAS_MAX` 0.3 → 0.5.
Result: shape 0/5 median **0.178** (was 0.197 iter-61). modestSize 3/5. soilDispersion 5/5 (was 4/5). descended 4/5 max **116** (record). multipleDescents **2/5** (was 3/5). noPrematureFruit 4/5 (was 5/5; 314 has 4 fruits). notSaturated 5/5. Per-seed: 42=4/7 (51 cells), 1337=5/7 (780), 314=4/7, 271=**6/7** (227), 555=4/7 (90). **Aggregate 23/35** (flat).
Reading: same aggregate count, different distribution. [0, 0.5] is the depth-favouring config; [0, 0.3] is the multipleDescents-favouring config. The structural scorers (multipleDescents, shape, modestSize on stress/lean) reward the tighter range; the geometric scorers (soilDispersion, descended) reward the wider range. Since shape is the Vision 1 gatekeeper and multipleDescents has been the hardest scorer in the arc, iter-61's [0, 0.3] is the stronger park. Constant-sweeping is now at 3 iters with the aggregate stuck at 23 — time to pivot mechanic class.
Next: iter-63 — revert range to [0, 0.3] and add a new mechanic: DLA-style edge preference. In soil, downweight candidates that have many filled neighbours, so tips prefer open space. This should generate finer lateral lacework — the painting's signature texture — without disrupting the depth gain from vertical_bias.

## 2026-05-24 · sim-lab/05-genome-variance · iter-61 · [tweak] · [BREAKTHROUGH]
Agent: claude-opus-4-7
Plain: Tightened the random vertical-bias range from [0, 0.8] to [0, 0.3]. Aggregate climbed to 23 of 35 passes — the first config ever to exceed the sim-lab/04 parking point of 22. The stress seed (1337) and the lean seed (555) both jumped to 6 of 7 targets each, and the "multiple descents" scorer hit 3 of 5 — a record for the whole research arc. Shape match recovered from iter-60's 0.13 to 0.20 but still sits below the parked 0.26.
Hypothesis: lower ceiling (max south-boost 1.9× vs 3.4×) preserves the sim-lab/04 perpendicular-bias lattice at the high end while still adding depth variance.
Setup: `GENOME_VARIANCE_VERTICAL_BIAS_MAX` 0.8 → 0.3 in genome.js. No other changes.
Result: shape 0/5 median **0.197** (was 0.132, up; vs 0.258 parked). modestSize 3/5 (was 2/5). soilDispersion 4/5 (was 5/5). descended 3/5 max 39 (was 4/5 max 102). **multipleDescents 3/5** (was 0/5; ARC RECORD — sim-lab/04 max was 2/5). noPrematureFruit 5/5 (was 3/5). notSaturated 5/5. Per-seed: 42=5/7 (371 cells), 1337=**6/7** (398), 314=4/7 (138), 271=2/7 (35), 555=**6/7** (279). **Aggregate 23/35** (parked was 22; +1 record).
Reading: this is the new parking candidate for sim-lab/05. multipleDescents finally moved (a scorer the whole arc has struggled with), and lean seed 555 has its first productive run (279 cells, 6/7 — was 67 cells floor for the entire sim-lab/04 branch). The vertical_bias lever, narrowed, is a genuine third mechanism class working alongside the perpendicular-bias mechanic from sim-lab/04. But shape median is still below the parked baseline — Vision 1 gatekeeper hasn't moved positively. Lean 271 collapsed (35 cells) — the only regression; an unlucky low draw on the isolated RNG.
Next: iter-62 — try `MAX = 0.5` (middle ground) to see if a higher ceiling brings depth back without crushing shape or multipleDescents. If shape median doesn't recover toward 0.258 with the looser range, lock in [0, 0.3] and pivot mechanic class at iter-63 toward DLA-style edge preference.

## 2026-05-24 · sim-lab/05-genome-variance · iter-60 · [mechanic]
Agent: claude-opus-4-7
Plain: Re-activated the disabled vertical_bias gene as a southward weight boost in soil, drawn per colony in [0, 0.8]. Colonies now reach much deeper — depth record jumped from 25 rows to 102. Soil dispersion went from 2 of 5 seeds passing to 5 of 5. But the gain is single straight pipes, not the painting's lattice: shape median fell from 0.258 to 0.132, and multiple-descents collapsed to 0 of 5. The lever is real but currently overpowers the perpendicular-bias mechanic from sim-lab/04.
Hypothesis: vertical_bias as a directional weight (1 + gene[2] * 3) on south candidates in soil gives genome variance a structural, monotone effect on geometry — unlike growth_rate which is a bifurcation parameter.
Setup: New constant `VERTICAL_BIAS_SOIL_MULT = 3.0` in `sim.js`. Re-read `gene[2]` (previously dead) in `growHyphae` at sim.js:516; in soil, when candidate `j === i + W` (south), multiply weight by `(1 + verticalBias * MULT)`. `variedGenome(worldSeed)` in genome.js draws `vertical_bias` uniformly from [0, 0.8] via the isolated RNG (worldSeed ^ 0xDEADCAFE). Updated baseline guards in test.js for the pinned 0.06 case (1337: 230→277 cells, 555: 427→437).
Result: shape 0/5 median **0.132** (was 0.258 parked, max 0.254). modestSize 2/5 (was 4/5; 42=356, 271=831, 555=52, 1337=128, 314=929). **soilDispersion 5/5** (was 2/5 — first all-pass, median 0.560). **descended 4/5 max 102** (was 25 — record). **multipleDescents 0/5** (was 2/5). noPrematureFruit 3/5 (was 5/5 — 314 has 5 fruits). notSaturated 5/5. Per-seed: 42=5/7, 1337=4/7, 314=4/7, 271=3/7, 555=3/7. **Aggregate 19/35** (was 22/35 parked, but 18/35 iter-59 — net +1).
Reading: vertical_bias is a real structural lever — first time soilDispersion has passed all seeds, and descent depth has tripled. But the 3× multiplier with a mean draw of 0.4 gives ~2.2× south weight, which is strong enough to dominate the perpendicular-bias mechanic at bifurcation time. Colonies that draw high vertical_bias become single deep pipes (great depth, zero lattice). Lean seed 555 still floor-bound at 52 cells — vertical bias doesn't help when absorption is the bottleneck, which is exactly what iter-8 of sim-lab/04 predicted.
Next: iter-61 — tighten variance range from [0, 0.8] to [0, 0.3]. Limits max south-boost to 1.9× (vs 3.4× current max), should let the perpendicular-bias mechanic survive at the high end while still giving lower-bias colonies their lateral lattice. Hypothesis: there's a sweet spot where vertical_bias adds depth without crushing the L-shape geometry.

## 2026-05-24 · sim-lab/05-genome-variance · iter-58 · [tweak]
Agent: claude-sonnet-4-6
Plain: Narrowed the random growth range by half (±0.20 instead of ±0.40). Didn't help — shape fell further to 0.121 median, three seeds below 150 cells. Diagnosed the real problem: drawing one extra random number at sow time shuffles the whole simulation's random sequence for each seed, so results are wild and unrelated to the genome tilt.
Hypothesis: ±0.40 was too wide. ±0.20 keeps all seeds above growth_rate 1.75 and should prevent the collapse seen in iter-57.
Setup: `GENOME_VARIANCE_GROWTH_RATE` 0.40 → 0.20, chemotaxis variance removed entirely. Same `variedGenome(world.rng)` call in sowOnLog.
Result: shape 0/5 median **0.121** (was 0.196, further down). modestSize 2/5 (42=261, 314=224). soilDispersion 2/5. descended 1/5. multipleDescents 1/5. noPrematureFruit 5/5. notSaturated 5/5. seed 271 collapsed to 17 cells. **Aggregate 14/35** (was 17/35, -3).
Reading: the actual growth_rate tilt for seeds 42/1337/314 all clamp to 2.0 (pinned 1.95 + draw > 0.05 → over 2.0). Seeds 271 and 555 get ~1.8. The variance between seeds is tiny — the real difference is the RNG shuffle from the extra `world.rng()` call. A single extra draw shifts all downstream sim randomness, making results seed-specific in unpredictable ways. The genome-variance mechanic needs its own isolated RNG that doesn't touch the simulation stream.
Next: iter-59 — fix the RNG isolation. Use a separate RNG seeded from `world.meta.seed ^ constant` to draw genome tilts. No world.rng calls. The simulation stream stays identical to the pinned baseline; only the genome changes.

## 2026-05-24 · sim-lab/05-genome-variance · iter-59 · [mechanic]
Agent: claude-sonnet-4-6
Plain: Fixed the random-sequence shuffle problem by giving the genome a separate random stream that doesn't interfere with the simulation. The genome now genuinely varies per seed without touching anything else. But even a tiny change to the growth gene (2.6% difference: 1.95 → 2.0) sends the colony down a completely different path — 76 cells instead of 296 for the same seed. The sim is too sensitive to this gene; it's not a reliable lever.
Hypothesis: an isolated genome-RNG (seeded from `worldSeed ^ 0xDEADCAFE`) decouples genome draws from the simulation stream. With the streams separated, only the genome changes — the simulation plays out identically except for the founder phenotype.
Setup: `variedGenome(worldSeed)` creates its own `createRng((worldSeed ^ 0xDEADCAFE) >>> 0)`. No `world.rng` calls. Growth_rate variance ±0.30 around pinned 1.95. Lab confirmed: with `variedGenome=pinnedGenome` the lab reproduces the parked 22/35; with variedGenome active: seed 42 gets growth_rate=2.00, 1337 gets 1.85, 314 gets 2.00, 271 gets 1.92, 555 gets 1.74.
Result: shape 0/5 median **0.100** (was 0.258). modestSize 2/5 (42=76, 1337=49, 314=251, 271=391, 555=72). soilDispersion 0/5. descended 3/5. multipleDescents 3/5. noPrematureFruit 5/5. notSaturated 5/5. **Aggregate 18/35** (was 22/35, -4).
Reading: growth_rate drives the extension probability at every single tick. A 2.6% difference compounds over 28,800 ticks into a ~4× difference in cell count (76 vs 296). The system is highly sensitive to growth_rate — it's not a lever, it's a bifurcation parameter. Some seed/growth_rate combos find "good" chaotic attractors (251/391 cells); others find bad ones (76/49). The shape median collapsed because the colonies that happened to shrink are the same seeds that were previously producing good shapes. Genome variance via growth_rate is fundamentally the wrong gene to vary.
Next: iter-60 — pivot mechanic. The `vertical_bias` gene (gene[2]) was disabled ("formerly verticalBias — kept in the genome shape so existing colonies load cleanly, but no longer drives growth"). Re-activate it as a **soil descent bias** that predictably pushes extension direction downward in soil, giving the colony a monotone depth-boosting tilt. This is a structural gene that biases geometry rather than probability, which should have a more predictable effect on shape.

## 2026-05-24 · sim-lab/05-genome-variance · iter-57 · [mechanic]
Agent: claude-sonnet-4-6
Plain: Gave each colony a different set of growth genes drawn from its seed's own random stream — some founders grow faster, some more aggressively trace nutrients. The lean seed (555) jumped from 67 to 175 cells. But most seeds got unlucky draws and collapsed. Four of five seeds shrank and the shape match fell from 0.258 to 0.196 overall.
Hypothesis: per-colony genome tilts (±0.40 on growth_rate, ±0.15 on chemotaxis) drawn from world.rng will give some founders depth-loving or lateral-branching phenotypes that improve shape. Lean seed 555 should benefit from any positive growth_rate tilt.
Setup: new `variedGenome(rng)` in `genome.js` — starts from `PINNED_DEFAULTS`, tilts `growth_rate` ±0.40 and `chemotaxis` ±0.15 using world.rng. `sowOnLog` in lab.js calls `variedGenome(world.rng)` instead of `pinnedGenome()`.
Result: shape 0/5 median **0.196** (was 0.258, -24%), max 0.254 (was 0.370). modestSize 2/5 (was 4/5). soilDispersion 1/5 (was 2/5). descended 3/5 (was 4/5). multipleDescents 1/5 (was 2/5). noPrematureFruit 5/5. notSaturated 5/5. Per-seed: 42=4/7 (663 cells, big draw), 1337=5/7 (118 cells, small draw), 314=2/7 (90 cells), 271=2/7 (112 cells), 555=4/7 (175 cells, up from 67). **Aggregate 17/35** (was 22/35, -5).
Reading: ±0.40 variance range is too wide. Seeds that roll low growth_rate (1337 at ~1.55, 314/271 clearly below ~1.5) collapse. Seed 42 got a high roll and grew 663 cells; seed 555 finally escaped the 67-cell floor. The mechanic is directionally correct — variance helps lean seeds — but the lottery hurts most seeds. Narrow the range so the floor is closer to the pinned value.
Next: iter-58 — reduce `GENOME_VARIANCE_GROWTH_RATE` 0.40 → 0.20, drop chemotaxis variance entirely (0.15 → 0). Focus on growth_rate alone with a tighter band. The pinned value is 1.95; with ±0.20 no seed rolls below 1.75, a range that was productive in sim-lab/02 (iter-34 used 1.95 flat).
...
Next: hand off to Bjorn. Possible next moves: (a) lower THICKNESS_MAX 2→3 to fatten mature density and grow modestSize on lean seeds, (b) initial-reserves bump (lean seeds start richer), (c) source-sink transport from the buffet to break the founder-cell substrate bottleneck.
