// Shroom — kit · tweaks-panel
// Floating panel of live-tweak controls for the /preview workshop.
// Adapted from an earlier tweaks-panel pattern.
// No host protocol, no persistence — purely local React state.
// Depends on: nothing (standalone, no kit deps).
...
const __TWEAKS_STYLE = `
  ...
  .twk-reopen{position:fixed;right:16px;bottom:16px;z-index:2147483646;
    ...
    background:rgba(250,249,247,.78);color:rgba(41,38,27,.8);
    -webkit-backdrop-filter:blur(24px);backdrop-filter:blur(24px);
    border:.5px solid rgba(255,255,255,.6);
    box-shadow:0 4px 16px rgba(0,0,0,.14);
    font:12px/1 ui-sans-serif,system-ui,-apple-system,sans-serif;font-weight:500;
    letter-spacing:.01em}
  .twk-reopen:hover{background:rgba(250,249,247,.95)}
`;

// ── useTweaks ────────────────────────────────────────────────────────────────
// Local-state-only tweak store. No persistence — reload resets to defaults.
...
// ── TweaksPanel ───────────────────────────────────────────────────────────────
// Draggable floating panel. Starts open; close button hides it;
// a small ⚙ button in the same corner reopens it.
