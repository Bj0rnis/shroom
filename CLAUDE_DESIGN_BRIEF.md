# Almari Shroom — Visual Treatment Brief v2 (for Claude.ai design)
...
This is a **separate game art kit** — not the Almari dashboard kit. Pixel
native. Inspiration from Almari's mood (dark, calm, restrained) welcome,
...
The canvas world has its own palette — not Almari dashboard tokens. The
right column (chronicle + hall) takes design cues from the persona, not
from the Almari kit. Propose a canvas palette spec:
...
**One** screenshot is attached: a current view of `https://shroom.almari`
...
- Don't try to make the chronicle panel "modern" — we want it to feel
  ancient and organic.
- Don't tie the canvas palette to the Almari dashboard kit — they're
  separate visual universes.

---

*If you're a future Claude reading this from the design session: the live
world is at `https://shroom.almari` (Tailscale-only). The author is on
