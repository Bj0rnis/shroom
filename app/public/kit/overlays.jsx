...
function HallTrigger({ entries, onOpen }) {
  ...
  return (
    <DarkPanel seed={13} elevation={2} style={{ color: COL.textMid, flexShrink: 0 }}>
      <button
        type="button"
        onClick={onOpen}
        title="Hall of fame"
    ...
    <div onClick={onClose} style={{
      ...
      <DarkPanel seed={2} onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }} />
          {msg && <span style={{ fontFamily: MONO, fontSize: 10, color: msg === 'ok' ? '#8aaa78' : COL.danger }}>{msg}</span>}
        ...
        <div style={{
          ...
          {status && (
            <div style={{ fontFamily: MONO, fontSize: 10, color: COL.dimLo, marginTop: 10, paddingTop: 10, borderTop: '1px dotted #2a261f' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '7px 0' }}>
      ...
      <button
        ...
        style={{
          ...
          background: v ? COL.faint : '#1f1c17', position: 'relative',
        }}>
        <span style={{
...
function _dashBtn(variant, extra) {
  ...
  if (variant === 'warn')   return { ...base, background: COL.inkDeep, color: COL.ember, border: '1px solid #5a4632' };
  if (variant === 'danger') return { ...base, background: COL.danger, color: COL.inkDeep, border: '1px solid #5a3232' };
...
function DevDashboardTrigger({ onOpen }) {
  ...
  const c = hover ? COL.ember : COL.dim;
  return (
...
function NavLinkTrigger({ href, title, label, draw }) {
  const [hover, setHover] = React.useState(false);
  const c = hover ? COL.ember : COL.dim; // ember on hover, dim default
  return (
